...
const Page = () => {
  ...
  const handleCancel = () => {
    if (data?.isEditMode && data.rawPayload.id) {
      router.push(`/admin-user-maintenance/rule-management/add-role?id=${data.rawPayload.id}`)
    } else {
      router.push('/admin-user-maintenance/rule-management/add-role')
    }
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push('/admin-user-maintenance/rule-management')
    } finally {
      setSubmitting(false)
    }
  }
